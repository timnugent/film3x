...
public:
	...
	PDB() : force_span(false), polar_flg(false), output_all_chains(false), beta(false), flip(false), pre_pos(false), pre_flip(false), verbose(true), max_x(-10000.0), max_y(-10000.0), max_z(-10000.0), max_c_dist(0.0), x_rot_opt(0.0), y_rot_opt(0.0), z_trans_opt(0.0), y_rot_pre(0.0), z_rot_pre(0.0), z_trans_pre(0.0), best_extra_shift(0.0), best_cyto_shift(0.0) {};
	~PDB(){};
	...
	std::vector<double> origin_shift();
...
private:
	...
	bool force_span, polar_flg, output_all_chains ,beta, flip, pre_pos, pre_flip, verbose;
