...
void GA::init(){
	...
	for (int j = 0; j < genlen; j++){
		curpool[0].genome.push_back(0.0);
	}
...
vector<double> GA::run_ga(){
    ...
    for (gen = 1;; gen++){
		...
		if(gen - prevgen > 50 || worst == best || ncalls > max_ga_calls){
	    	if(verbose) cout << "*** Convergence detected!" << endl;
	    	return(results);
		}	
		temp = newpool;
		newpool = curpool;
...
double GA::eval(vector<double>& t, int thread){
	...
	if (v < vbest){
		if(threads > 1){
			if(verbose) printf("Lowest energy %f after %d function evaluations (thread %d).\n", v, ncalls, thread+1);
		}else{
			if(verbose) printf("Lowest energy %f after %d function evaluations.\n", v, ncalls);
		}
		results = t;
		vbest = v;
	}
